...
#define NUM_BYTES ((ADDRESS_MASK + 1) - BASE_ADDRESS)
 ...
 *              possible.
 *
 * Returns:     Zero if the test succeeds.  
...
unsigned int nBytes;
{
    unsigned int addressMask = ADDRESS_MASK;
    unsigned int offset;
    unsigned int testOffset;

    datum pattern     = (datum) 0xAA;
    datum antipattern = (datum) 0x55;
...
{
    unsigned int offset;
    unsigned int nWords = nBytes / sizeof(datum);
