# Brief: migrate web-sw-cor24-ocaml off the removed internal assembler

**Owner:** dwoca
**Branch:** `feat/migrate-to-cor24-assembler` → `pr/migrate-to-cor24-assembler`
**Repo:** `web-sw-cor24-ocaml`
...
which no longer exists, so the build fails. OCaml uses the assembler **only at
build time** (it pre-assembles the prelude in `build.rs`).

## Affected sites

- `build.rs:11` — `let mut asm = cor24_emulator::Assembler::new();`
...
2. `build.rs:11`: change `cor24_emulator::Assembler::new()` →
   `cor24_assembler::Assembler::new()`. (Fully-qualified, one-token change; no
   `use` statement to touch.)
...
`dg-relay dwoca web-sw-cor24-ocaml pr/migrate-to-cor24-assembler`.
