...
accumulated 6 brief(s) addressed to you. Until now they've lived
...
6 new untracked file(s) under `docs/briefs/`, named
`<N>-dcemu-<slug>.md`:

- `N` is the chronological order (by file mtime) the brief was drafted.
- The original filename is preserved (with the `dcemu-` prefix kept so the file is self-identifying when read out of context).
...
git commit -m "docs(briefs): capture historical briefs (6 files)"
...
- All 6 files committed under `docs/briefs/`.
