...
accumulated 20 brief(s) addressed to you. Until now they've lived
...
20 new untracked file(s) under `docs/briefs/`, named
`<N>-dcsno-<slug>.md`:

- `N` is the chronological order (by file mtime) the brief was drafted.
- The original filename is preserved (with the `dcsno-` prefix kept so the file is self-identifying when read out of context).
...
git commit -m "docs(briefs): capture historical briefs (20 files)"
...
- All 20 files committed under `docs/briefs/`.
