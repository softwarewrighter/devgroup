...
accumulated 3 brief(s) addressed to you. Until now they've lived
...
3 new untracked file(s) under `docs/briefs/`, named
`<N>-dwxas-<slug>.md`:

- `N` is the chronological order (by file mtime) the brief was drafted.
- The original filename is preserved (with the `dwxas-` prefix kept so the file is self-identifying when read out of context).
...
git commit -m "docs(briefs): capture historical briefs (3 files)"
...
- All 3 files committed under `docs/briefs/`.
