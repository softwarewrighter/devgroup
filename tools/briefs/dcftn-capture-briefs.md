...
accumulated 4 brief(s) addressed to you. Until now they've lived
...
4 new untracked file(s) under `docs/briefs/`, named
`<N>-dcftn-<slug>.md`:

- `N` is the chronological order (by file mtime) the brief was drafted.
- The original filename is preserved (with the `dcftn-` prefix kept so the file is self-identifying when read out of context).
...
git commit -m "docs(briefs): capture historical briefs (4 files)"
...
- All 4 files committed under `docs/briefs/`.
