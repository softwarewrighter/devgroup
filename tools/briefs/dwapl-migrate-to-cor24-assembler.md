# Brief: migrate web-sw-cor24-apl off the removed internal assembler
...
which no longer exists, so the build fails. APL uses the assembler **only at build
time** (it pre-assembles the prelude in `build.rs`).

## Affected sites

- `build.rs:7` — `let mut asm = cor24_emulator::Assembler::new();`
...
2. `build.rs:7`: change `cor24_emulator::Assembler::new()` →
...
`dg-relay dwapl web-sw-cor24-apl pr/migrate-to-cor24-assembler`.
