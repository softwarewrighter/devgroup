...
accumulated 9 brief(s) addressed to you. Until now they've lived
...
9 new untracked file(s) under `docs/briefs/`, named
`<N>-dcpls-<slug>.md`:

- `N` is the chronological order (by file mtime) the brief was drafted.
- The original filename is preserved (with the `dcpls-` prefix kept so the file is self-identifying when read out of context).
...
git commit -m "docs(briefs): capture historical briefs (9 files)"
...
- All 9 files committed under `docs/briefs/`.
