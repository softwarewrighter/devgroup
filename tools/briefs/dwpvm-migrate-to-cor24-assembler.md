# Brief: migrate web-sw-cor24-pcode off the removed internal assembler
...
which no longer exists, so the build fails. P-code uses the assembler **only at
...
2. `build.rs:10`: change `cor24_emulator::Assembler::new()` →
...
`dg-relay dwpvm web-sw-cor24-pcode pr/migrate-to-cor24-assembler`.
