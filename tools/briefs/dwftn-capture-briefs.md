...
1 new untracked file(s) under `docs/briefs/`, named
`<N>-dwftn-<slug>.md`:

- `N` is the chronological order (by file mtime) the brief was drafted.
- The original filename is preserved (with the `dwftn-` prefix kept so the file is self-identifying when read out of context).
...
git commit -m "docs(briefs): capture historical briefs (1 files)"
...
- All 1 files committed under `docs/briefs/`.
