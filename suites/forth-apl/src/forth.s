...
word_skip:
    la r0, broker_getc
    la r2, word_skip_return
    add r1, -3
    sw r2, 0(r1)
    jmp (r0)
word_skip_return:        ; r0 = char
...
word_read:
    la r0, broker_getc
    la r2, word_read_return
    add r1, -3
    sw r2, 0(r1)
    jmp (r0)
word_read_return:        ; r0 = char
    lcu r2, 33
    clu r0, r2           ; C = (char < 33)
...
; ============================================================
; ============================================================
; Monitor-owned UART RX broker
; ============================================================
; Called with `jal fp,(r0)`. Returns one non-attention byte in r0.
; The resident monitor ISR is the sole producer; Forth is the sole consumer.
broker_getc:
broker_getc_wait:
    la r0, 2054          ; monitor abort flag at 0x000806
    lbu r0, 0(r0)
    ceq r0, z
    brt broker_getc_poll
    la ir, 0             ; non-local restart at monitor entry
broker_getc_poll:
    la r0, 2048          ; ring head byte at 0x000800
    lbu r0, 0(r0)
    la r2, 2051          ; ring tail byte at 0x000803
    lbu r2, 0(r2)
    ceq r0, r2
    brt broker_getc_wait
...
; Runtime definitions begin after the broker, so compiled words cannot
; overwrite code used to receive input.
dict_end:
